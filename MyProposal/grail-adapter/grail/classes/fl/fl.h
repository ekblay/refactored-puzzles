...
template <class Item>
std::istream& operator >> ( std::istream&, fl<Item>&);

template <class Item>
std::ostream& operator << ( std::ostream&, const fl<Item>&); //by SH 05/25/2005
...
#ifdef _MSC_VER
	#if _MSC_VER<=1200
	friend	std::ostream& operator<<(std::ostream&, const fl<Item>&);
	friend	std::istream& operator>>(std::istream&, fl<Item>&);
	#endif

	#if _MSC_VER>=1300
    friend std::ostream& operator << <Item> ( std::ostream&, const fl<Item>&);
	friend std::istream& operator >> <Item> ( std::istream&, fl<Item>&);
	#endif
#endif
...
public:
	...
	void		get_alphabet(set<Item> &s) const;
			// retrieves the set of symbols which make
			// up the words in this language.
	...
	fl<Item>&	operator-=(const string<Item>& s)
        ...
        bool           is_sim(string<Item>& a,string<Item>& b) const;

        void           flsimset() const;

       bool            is_equivalent(string<Item>& a,string<Item>& b) const;

        void           fleqset() const;

        void         inttofl(int alphabet_size,char * num, int num_length) ;
        void            fltoint(int alpha_size) ;
};
