...
public:
	...
	array<Item>&	clear()
        ...
        array<Item>&	clear_all()
                        // clear the contents of this array and free memory
			{ sz = 0; sorted = 1; delete []p; p=new Item[max= ARRAY_SIZE]; return *this; }
	...
	static	int	compare(const Item*, const Item*);
                        ...
                        //  otherwise
	static	int	compare_length(const Item*, const Item*);
                        // compare method, length considered first
