...
int main(int argc, char const *argv[]) {
    ...
    while (1) {
        ...
        if ((stringBuf.compare(CLIENT_PUZZLE)) == 0 || (stringBuf.compare(CLIENT_PUZZLE_RETRY)) == 0) {
            ...
            string solvedPuzzle = clientCrypto.calculateSolution(clientPuzzle, solution, numberOfMissingCharacters,
                                                                 maxIterations);

            //send out
            send(sock, (MESSAGE_HEADER + CLIENT_PUZZLE_SOLUTION).c_str(),
                 (MESSAGE_HEADER + CLIENT_PUZZLE_SOLUTION).length(), 0);
            send(sock, (DATA + clientCrypto.getPuzzleSolutionPayload()).c_str(),
                 (DATA + clientCrypto.getPuzzleSolutionPayload()).length(), 0);
